...
const toLastNames = people => {
  // Solution code here...
  let newArr = people.map( (person, idx) => person.firstName + ' ' + person.lastName);
  return newArr;
};
...
const addValues = (arr) => {
  // Solution code here...
  let newValue = arr.reduce ( (addNum, num) => {
    addNum = num + addNum
    return addNum;
  }, 0)
  return newValue;
...
const reversedString = (str) => {
  ...
  let newArr = string.reduce( (acc, str) => {
    return str + acc;
  },'');
  return newArr;
};
